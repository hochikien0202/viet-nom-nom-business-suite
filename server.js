...
const V160_HOT_READY_COMBOS = [
  ...
  { id:'combo-single-item', name:'C2. Single Rice or Noodles', category:'Combo', price:5.99, description:'Choose one serving of fried rice, chow mein, or Ho-Fun.', active:true, soldOut:false, cost:0, image:'' },
...
const V160_HOT_READY_SELECTIONS = [
  ...
  { id:'hot-base-ho-fun', name:'Ho-Fun', viName:'Ho-Fun (Bánh phở xào)', category:'Hot & Ready Base', price:0, description:'Hot & Ready base selection.', active:true, soldOut:false, cost:0, image:'' },
...
async function createOrderFromPayload(b, { source = 'website', allowOverride = false, initialStatus = 'new' } = {}) {
  ...
  const requestedFulfillment = String(b.fulfillment || b.channel || 'pickup').toLowerCase();
  const fulfillment = source === 'website' ? 'pickup' : requestedFulfillment;
  ...
  const o = { id: id(), orderNo: orderNo(), createdAt: new Date().toISOString(), updatedAt: new Date().toISOString(), source, channel: txt(b.channel || source, 40), status: initialStatus, customer, items, fulfillment: ['pickup', 'delivery', 'walk-in', 'phone'].includes(fulfillment) ? fulfillment : 'pickup', address: txt(b.address, 300), requestedTime: txt(b.requestedTime, 60) || 'ASAP', payment: txt(b.payment, 60) || 'Cash', paymentStatus: txt(b.paymentStatus, 30) || 'unpaid', notes: txt(b.notes, 1000), staffNote: '', promotionId: txt(b.promotionId, 100), manualDiscount: money(Math.max(0, Number(b.manualDiscount) || 0)), taxExempt: !!b.taxExempt, customerDistanceKm: Math.max(0, Number(b.distanceKm || b.customerDistanceKm) || 0), finalDistanceKm: Math.max(0, Number(b.finalDistanceKm || b.distanceKm || b.customerDistanceKm) || 0), saleId: null };
  recalcOrder(o, promotions, source === 'website' ? 'website' : 'pos'); return o;
}

app.get('/api/health', async (req, res, next) => { try { const h = await db.healthCheck(); res.setHeader('Cache-Control','no-store'); res.json({ ok: true, time: new Date().toISOString(), version: '2.0.0', storage: db.storageLabel || 'Supabase PostgreSQL', databaseTime: h.serverTime, adminPinSource: process.env.ADMIN_PIN ? 'environment' : 'default' }); } catch (e) { next(e); } });
...
app.post('/api/admin/login', (req, res) => { res.setHeader('Cache-Control','no-store'); return normalizePin(req.body?.pin) === ADMIN_PIN ? res.json({ ok: true, version: '2.0.0' }) : res.status(401).json({ error: 'Incorrect PIN' }); });
...
app.post('/api/orders', async (req, res, next) => { try {
  ...
  if (b.fulfillment === 'delivery' && settings.deliveryEnabled === false) return res.status(409).json({ error: 'Delivery is temporarily unavailable. Please use pickup.' });
