...
async function confirmDraft(){
  ...
  try{
    const r=await fetch('/api/orders',{method:'POST',headers:{'Content-Type':'application/json','Accept':'application/json'},cache:'no-store',body:JSON.stringify({customer:d.customer,items:d.items.map(x=>({id:x.id,qty:x.qty,displayName:x.name,modifiers:Array.isArray(x.modifiers)?x.modifiers:[]})),fulfillment:d.fulfillment,address:d.address||'',distanceKm:Number(d.distanceKm)||0,requestedTime:d.requestedTime||'ASAP',payment:d.payment,notes:composeOrderNotesV1135(d)})});
