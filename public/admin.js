...
saveOrderEdits=async function(markComplete=false){if(!editingOrder||!editCart.length)return toast('Order must contain at least one item.');const payload={items:editCart.map(x=>({id:x.id,qty:x.qty,priceOverride:x.price,displayName:x.name,modifiers:Array.isArray(x.modifiers)?x.modifiers:[]})),customer:{name:A('#editCustomerName').value,phone:A('#editCustomerPhone').value,email:A('#editCustomerEmail').value},promotionId:A('#editPromotion').value,manualDiscount:Number(A('#editManualDiscount').value)||0,payment:A('#editPayment').value,fulfillment:A('#editFulfillment').value,address:A('#editAddress').value,finalDistanceKm:Number(A('#editDistance').value)||0,notes:A('#editNotes').value};if(markComplete)payload.status='completed';const d=await req('/api/admin/orders/'+editingOrder.id,{method:'PATCH',body:JSON.stringify(payload)});editingOrder=d.order;toast(markComplete?'Order completed · thank-you update triggered':'Order checkout updated');if(markComplete)closeOrderEditor();await refresh();if(!markComplete&&editingOrder)openOrderEditor(editingOrder.id)};
...
saveOrderEdits=async function(markComplete=false){
  ...
  try{const payload={items:editCart.map(x=>({id:x.id,qty:x.qty,priceOverride:x.price,displayName:x.name,modifiers:Array.isArray(x.modifiers)?x.modifiers:[]})),customer:{name:A('#editCustomerName').value,phone:A('#editCustomerPhone').value,email:A('#editCustomerEmail').value},promotionId:A('#editPromotion').value,manualDiscount:Number(A('#editManualDiscount').value)||0,payment:A('#editPayment').value,fulfillment:A('#editFulfillment').value,address:A('#editAddress').value,finalDistanceKm:Number(A('#editDistance').value)||0,notes:A('#editNotes').value};if(markComplete)payload.status='completed';const d=await req('/api/admin/orders/'+editingOrder.id,{method:'PATCH',body:JSON.stringify(payload)});editingOrder=d.order;toast(markComplete?'Order completed':'Order updated successfully');if(markComplete){closeOrderEditor();await refresh()}else{const idx=orders.findIndex(x=>x.id===editingOrder.id);if(idx>=0)orders[idx]=editingOrder;renderEditor();renderOrders()}}
