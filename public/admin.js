...
function ensureOrderAlertAudioV115(){
  if(!orderAlertAudioV115){
    orderAlertAudioV115=new Audio('/assets/sounds/order-alert.mp3?v=2.0.0');
...
renderOrders=function(){
  ...
  A('#ordersGrid').innerHTML=list.map(o=>{
    ...
    return `<article class="order-card ${o.status==='new'&&due?'is-new':''} ${o.status==='new'&&!due?'is-scheduled':''} ${o.status==='ready'?'is-ready':''} ${requests.allergy.length?'has-allergy-alert':''}">
      ...
      <div class="order-actions"><button class="action-btn checkout" onclick="openOrderEditor('${o.id}')">Edit / Checkout</button>${['accepted','preparing','ready',o.fulfillment==='delivery'?'out-for-delivery':'completed','completed'].filter((v,i,a)=>a.indexOf(v)===i).map(st=>`<button class="action-btn ${st==='completed'?'secondary':''}" onclick="setStatus('${o.id}','${st}')">${st==='ready'?(o.fulfillment==='pickup'?'READY FOR PICKUP':'READY'):st==='completed'?'PAID & COMPLETED':st.replaceAll('-',' ')}</button>`).join('')}</div>
    </article>`;
  }).join('');
  A('#emptyState').hidden=!!list.length;A('#statNew').textContent=orders.filter(orderDueAdminV116).length;A('#statPreparing').textContent=orders.filter(o=>o.status==='preparing').length;A('#statReady').textContent=orders.filter(o=>['ready','out-for-delivery'].includes(o.status)).length;const today=new Date().toDateString();A('#statCompleted').textContent=orders.filter(o=>o.status==='completed'&&new Date(o.updatedAt).toDateString()===today).length;
};
...
let v1125SnapshotReady=false,staffSnapshotSignatureV200='';
...
refresh=async function(){
  ...
  try{
    ...
    const nextOrders=d.orders||[],nextMenu=d.menu||[],nextSettings=d.settings||{},nextPromotions=d.promotions||[];
    const signature=JSON.stringify([nextOrders,nextMenu,nextSettings,nextPromotions,d.status||{}]);
    const changed=signature!==staffSnapshotSignatureV200;
    orders=nextOrders;menu=nextMenu;settings=nextSettings;promotions=nextPromotions;window.providerInfo=d.notificationProviders||{};
    if(changed){staffSnapshotSignatureV200=signature;renderOrders();syncAlarm();renderStore(d.status||{});renderMenu();renderStaffPOS()}
...
function startStaffLiveSyncV1125(){
  if(window.adminTimer)clearInterval(window.adminTimer);
  window.adminTimer=setInterval(()=>{if(!document.hidden)refresh()},3000);
