...
function setupOrderPage(){
  ...
  function sync(){const mode='pickup',atRestaurant=mode!=='delivery';if(addressWrap)addressWrap.style.display='none';if(pickupPay)pickupPay.style.display='grid';if(deliveryPay)deliveryPay.style.display='none';const payTitle=$('#restaurantPaymentTitle');if(payTitle)payTitle.textContent='Payment — Pickup';const desired=mode==='delivery'?'cash-driver':'counter',checked=$('input[name="payment"]:checked');if(!checked||((atRestaurant&&checked.value==='cash-driver')||(mode==='delivery'&&checked.value==='counter'))){const fallback=$(`input[name="payment"][value="${desired}"]`);if(fallback)fallback.checked=true}syncDeliveryEstimate();renderCart()}
  ...
  $('#placeOrder')?.addEventListener('click',async()=>{
    ...
    const name=$('#customerName')?.value.trim()||'',phone=$('#customerPhone')?.value.trim()||'',email=$('#customerEmail')?.value.trim()||'',mode='pickup',payment=$('input[name="payment"]:checked')?.value||'';
...
renderCart=function(){
  ...
  const foodSubtotal=cart.reduce((sum,x)=>sum+Number(x.price)*x.qty,0),mode='pickup',km=Number($('#deliveryDistance')?.value)||0,delivery=mode==='delivery'?calcDeliveryFee(km):0,tax=foodSubtotal*.13,foodTotalAfterTax=foodSubtotal+tax,total=foodTotalAfterTax+delivery;
...
setupOrderPage=function(){
  ...
  function syncFulfillment(){
    const mode='pickup',isDelivery=mode==='delivery';
    ...
    const payTitle=$('#restaurantPaymentTitle');if(payTitle)payTitle.textContent='Payment — Pickup';
    // Direct website ordering is pickup only; counter payment remains selected.
  ...
  $('#placeOrder')?.addEventListener('click',async()=>{
    const cart=getCart();if(!cart.length){toast('Please add at least one item first.');return}
    const name=$('#customerName')?.value.trim()||'',phone=$('#customerPhone')?.value.trim()||'',email=$('#customerEmail')?.value.trim()||'',mode='pickup',payment=$('input[name="payment"]:checked:not(:disabled)')?.value||'',timing=$('#orderTimingMode')?.value||'asap';
...
function restoreCheckoutDraftV1120(){
  if(!$('#continueCheckout'))return;const d=currentCheckoutDraftV1120();if(!d)return;d.fulfillment='pickup';d.address='';d.distanceKm=0;d.payment='counter';
